...
class Spider(object):
    ...
    def __init__(self):
        ...
        self.collection_two = MongoClient(MONGO_CONNECTION_STRING)[MONGO_DB_NAME][MONGO_COLLECTION_NAME]
    ...
    def save_two(self, data):
        logging.info('saving data %s', data)
        if data:
            self.collection_two.update_one({
                'id': data.get('id')
            }, {
                '$set': data
            }, upsert=True)
